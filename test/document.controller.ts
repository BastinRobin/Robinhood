...
describe('Document', () => {
  ...
  it('should not get an document when id is null', () =>
    request(Server)
      .get(`/v1/document/null`)
      .set({ token: process.env.UNIT_TESTING_AUTH_TOKEN })
      .expect('Content-Type', /json/)
      .then((r) => {
        expect(r.body).equal(
  ...
  it('should not get an document when url not found', () =>
    request(Server)
      .get(`/v1/documents/`)
      .set({ token: process.env.UNIT_TESTING_AUTH_TOKEN })
  ...
  it('should not delete document when id is null', () =>
    request(Server)
      .delete(`/v1/document/null`)
      .set({ token: process.env.UNIT_TESTING_AUTH_TOKEN })
      .expect('Content-Type', /json/)
      .then((r) => {
        expect(r.body).equal(
